...
export default class UserStoriesStore {
	...
	@action.bound getUserStoriesResponse = (
		response: userStoriesResponseTypes,
	): void => {
		const userStoriesModels = response.users_stories.map(
			(eachStory) => new UserStoryModel(eachStory),
		);
