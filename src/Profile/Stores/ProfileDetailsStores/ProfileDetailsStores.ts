...
class ProfileDetailsStores{
    ...
    @action.bound getProfileDetailsResponse = (response: profileAndResponseStatusTypes): void => {
    ...
    @action.bound getProfileDetailsStatus = (responseStatus: APIStatus): void => {
		this.profileDetailsStatus = responseStatus;
    }

    fetchProfileDetails = () => {
        const profileDetails = this.profileDetailsApiService.getProfileDetails();
